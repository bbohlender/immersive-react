...
- hover, and highlight when selected
    - add `const selected = useStore((state) => state.selected === name);
    const ref = useRef<Mesh>(null);
    const hovered = useHover(ref);` into the Model component
    - set ref on mesh in the Model component
    - add onto mesh in the Model component
      material-emissive={selected || hovered ? color : 0x0}
      material-emissiveIntensity={selected ? 0.6 : 0.3}
- vibrate when hovering over different shoe element or over color in color picker
    - add `onPointerOver={(e) => {
                    if (
                      "pointerState" in e &&
                  ...
                  }}` to the Container in pickableColors.map and to the mesh in InteractableMesh
- sound when selecting shoe element and when changing the color
    - in model.tsx import `PositionalAudio` from drei and `import { PositionalAudio as PAudio } from "three";`
    - add a `const soundRef = useRef<PAudio>()` in the InstactableMesh Component
    - add the PositionalAudio component to the InstactableMesh Component with ref={soundRef}, url="https://www.immersive-react.com/select.ogg", loop={false} properties
    - call `soundRef.current?.play()` in the onClick handler of InstactableMesh
    - add the same imports to `color-picker.tsx`, add the PositionalAudio with the same properties into the Billboard tag of the ColorPicker component, and add the  `const soundRef = useRef<PAudio>()` into the ColorPicker component
    - call `soundRef.current?.play()` in the onClick handler of Container in pickableColors.map