...
First, we highlight the shoe parts when users hover over them.
We check if the current shoe part is selected from the `useColorStore` hook.
...
Next, we'll add haptic feedback for VR controllers when interacting with the shoe parts or the color picker. We trigger the `hapticActuators` of the hovering controller through the `onPointerOver` handler. In the handler we first check if the event is triggered by a XR controller before triggering its `hapticActuators`.
...
Lastly, we'll add audio feedback that plays when selecting shoe parts or changing colors. First, we add the `PositionalAudio` component in the `InteractableMesh` component, which allows to play sound or music from a 3D position in space.
```tsx
...
Next, we start playing the sound in the `onClick` handler in the `InterableMesh` through the `soundRef`.
...
Lastly, we apply a similar modification to the ColorPicker component, adding a `PositionalAudio` component inside the Billboard.
```tsx
const soundRef = useRef<PAudio>();
...
Finally, we similarly play the sound through `soundRef.current?.play()` in the `onClick` handler for changing the color of the selected shoe part.
