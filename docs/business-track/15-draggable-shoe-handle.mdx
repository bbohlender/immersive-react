...
export default function App() {
  return (<>
    ...
    <Canvas onPointerMissed={() => useStore.getState().setSelected(undefined)} style={{ position: "absolute", inset: "0", touchAction: "none" }}>
      <XR store={store}>
...
Next, we implement pointer event handling.
...
Lastly, we implement the `onPointerUp` handler to clean up the `draggingPointerId` when dragging ends:
```tsx
onPointerUp={() => {
  if (draggingPointerId.current == null) {
    return;
...
Finally, we add these handlers to our draggable handle mesh  and set its scaling to `0.1` which results in a sphere with `10cm` radius.
...
Finally, we use the Draggable component by wrapping the InteractablMeshes with it.
